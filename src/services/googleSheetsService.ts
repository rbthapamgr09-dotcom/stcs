 ...
 * Named "stcs_<office name>" and placed inside the designated Drive folder
 */
export async function createAppSpreadsheet(
  ...
  const cleanOfficeName = (orgName || 'कार्यालय').trim();
  const title = `stcs_${cleanOfficeName}`;
  ...
  try {
    ...
    if (orgValues.length > 1) {
      orgValues.forEach((row) => {
        ...
        if (key.includes('लोगो') || key.includes('Logo') || key === 'logoUrl') orgData.logoUrl = val;
