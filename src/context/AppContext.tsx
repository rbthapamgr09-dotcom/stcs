...
/**
 * Creates completely blank progressive tax slabs and statutory ceiling references for a new office.
 * Fields start unconfigured/blank until user explicitly inputs and saves to database.
 */
export const createBlankTaxReferencePair = (fiscalYear: string, orgId: string = 'org_default'): TaxReference[] => {
  const blankSlabs: TaxSlab[] = [
    { id: `slab_1_${Date.now()}`, description: '', fromAmount: undefined, toAmount: undefined, ratePercent: undefined },
    { id: `slab_2_${Date.now()}`, description: '', fromAmount: undefined, toAmount: undefined, ratePercent: undefined },
    { id: `slab_3_${Date.now()}`, description: '', fromAmount: undefined, toAmount: undefined, ratePercent: undefined },
    { id: `slab_4_${Date.now()}`, description: '', fromAmount: undefined, toAmount: undefined, ratePercent: undefined },
    { id: `slab_5_${Date.now()}`, description: '', fromAmount: undefined, toAmount: undefined, ratePercent: undefined },
    { id: `slab_6_${Date.now()}`, description: '', fromAmount: undefined, toAmount: undefined, ratePercent: undefined },
  ];

  const fyClean = (fiscalYear || 'fy').replace(/\//g, '_');
  return [
    {
      id: `tax_ref_single_${orgId}_${fyClean}`,
      fiscalYear,
      filingType: 'एकल',
      slabs: blankSlabs.map((s, i) => ({ ...s, id: `single_slab_${i + 1}_${Date.now()}` })),
    ...
    {
      id: `tax_ref_couple_${orgId}_${fyClean}`,
      fiscalYear,
      filingType: 'दम्पत्ती',
      slabs: blankSlabs.map((s, i) => ({ ...s, id: `couple_slab_${i + 1}_${Date.now()}` })),
      remoteExemptions: { 'क': undefined, 'ख': undefined, 'ग': undefined, 'घ': undefined, 'ङ': undefined, 'दुर्गम नभएको': undefined },
      disabilityExemptionPercent: undefined,
      femaleTaxRebatePercent: undefined,
      pensionSSTExempt: true,
...
interface AppContextType {
  ...
  saveTaxReferencesBatch: (taxRefs: TaxReference[]) => Promise<boolean>;
  ...
  createGoogleSpreadsheetForApp: (options?: {
    ...
    silent?: boolean;
    promptForOAuth?: boolean;
  }) => Promise<{ success: boolean; isScopeError?: boolean; spreadsheetId?: string; url?: string; message: string }>;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (
    ...
    // 2. Identify User in local cache
        ...
        if (cloudResult.success && cloudResult.user) {
          ...
          setUsers((prev) => {
            const updated = prev.filter(
              (u) => u.id !== found!.id && u.username.toLowerCase() !== found!.username.toLowerCase()
            );
            updated.push(found!);
            try {
  ...
  const setActiveFiscalYear = (newFy: string) => {
    ...
    const targetData = updatedDb[newFy] || {
      ...
      salarySetups: {},
      deductionSetups: {},
      taxReferences: defaultTaxRefsForTarget,
    };
    ...
    setTaxReferences(
      targetData.taxReferences && targetData.taxReferences.length > 0
        ? targetData.taxReferences
        : defaultTaxRefsForTarget
    );
  ...
  const saveTaxReferencesBatch = async (newRefs: TaxReference[]): Promise<boolean> => {
    if (!hasPermission('MANAGE_TAX_SLABS')) {
      addToast('error', 'अनाधिकृत कार्य', 'कर स्ल्याब परिवर्तन गर्न अधिकार छैन।');
      return false;
    }

    let updatedTaxReferences = [...taxReferences];
    for (const ref of newRefs) {
      const idx = updatedTaxReferences.findIndex(
        (t) => t.id === ref.id || (t.fiscalYear === ref.fiscalYear && t.filingType === ref.filingType)
      );
      if (idx >= 0) {
        updatedTaxReferences[idx] = ref;
      } else {
        updatedTaxReferences.push(ref);
      }
    }
    ...
    const updatedDb = {
      ...fyDatabase,
      [activeFiscalYear]: {
        ...(fyDatabase[activeFiscalYear] || { employees: [], salarySetups: {}, deductionSetups: {} }),
        employees,
        salarySetups,
        deductionSetups,
        taxReferences: updatedTaxReferences,
      },
    };
    setFyDatabase(updatedDb);
    ...
    const outgoingOrgUsers = users.filter(
      (u) => u.organizationId === activeOrganizationId || (!u.organizationId && activeOrganizationId === 'org_default')
    );

    const updatedStore: OrganizationDataStore = {
      organization,
      fiscalYears,
      activeFiscalYear,
      fyDatabase: updatedDb,
    ...
    setOrgDatabases((prev) => {
      const cur = prev[activeOrganizationId] || updatedStore;
      const nextStore = {
        ...cur,
        fyDatabase: updatedDb,
      };
      try {
        localStorage.setItem(STORAGE_KEYS.ORG_DATABASES, JSON.stringify({ ...prev, [activeOrganizationId]: nextStore }));
      } catch {}
      return { ...prev, [activeOrganizationId]: nextStore };
    });

    // Persist to Cloud specifically for this office and fiscal year
    saveCloudTaxReferences(updatedTaxReferences, activeOrganizationId, activeFiscalYear).catch(console.warn);
    saveCloudFyDatabase(updatedDb, activeOrganizationId).catch(console.warn);
    saveCloudOrgStore(activeOrganizationId, updatedStore).catch(console.warn);
  ...
  const deleteTaxReference = (id: string) => {
    ...
    saveCloudTaxReferences(updated, activeOrganizationId, activeFiscalYear).catch(console.warn);
    saveCloudFyDatabase(updatedDb, activeOrganizationId).catch(console.warn);
  ...
  const setActiveOrganizationId = (orgId: string) => {
    ...
    if (targetStore && targetStore.fyDatabase && Object.keys(targetStore.fyDatabase).length > 0) {
      ...
      const targetConfig: GoogleSheetsConfig = targetStore.googleSheetsConfig || {
        ...
        spreadsheetName: `stcs_${orgObj?.officeName || targetOrg.officeName || 'कार्यालय'}`,
        autoSync: true,
        syncMode: 'auto',
      };
            ...
            getCloudFyDatabase(orgId).then((cloudFyDb) => {
              if (cloudFyDb && Object.keys(cloudFyDb).length > 0) {
                ...
                if (fyData) {
                  ...
                  if (fyData.taxReferences && fyData.taxReferences.length > 0) {
                    setTaxReferences(fyData.taxReferences);
                  } else {
                    setTaxReferences(
                      orgId === 'org_default'
    ...
    // Also specifically query cloud tax-references for this org & fiscal year in case they were saved directly
    getCloudTaxReferences(orgId, '2083/084')
      .then((cloudTaxRefs) => {
        if (cloudTaxRefs && Array.isArray(cloudTaxRefs) && cloudTaxRefs.length > 0) {
          setTaxReferences(cloudTaxRefs);
        }
      })
      .catch(() => {});
    ...
    getOrgSheetsConfig(orgId)
      .then((cloudCfg) => {
        if (cloudCfg && (cloudCfg.webAppUrl || cloudCfg.spreadsheetId)) {
          setGoogleSheetsConfig((cur) => ({
            ...cur,
            webAppUrl: cloudCfg.webAppUrl || cur.webAppUrl,
            spreadsheetId: cloudCfg.spreadsheetId || cur.spreadsheetId,
            spreadsheetUrl: cloudCfg.spreadsheetUrl || cur.spreadsheetUrl,
            spreadsheetName: cloudCfg.spreadsheetName || cur.spreadsheetName || `stcs_${orgObj?.officeName || 'कार्यालय'}`,
          }));
        }
      })
      .catch(() => {});
  ...
  const updateGoogleSheetsConfig = (cfg: Partial<GoogleSheetsConfig>) => {
    setGoogleSheetsConfig((prev) => {
      const updated: GoogleSheetsConfig = { ...prev, ...cfg };

      // 1. Update active organization's database in orgDatabases
      setOrgDatabases((prevDbs) => {
        const curOrgDb = prevDbs[activeOrganizationId] || {
          organization,
          fiscalYears,
          activeFiscalYear,
          fyDatabase,
        ...
        const updatedDbs = {
          ...
          [activeOrganizationId]: {
            ...curOrgDb,
            googleSheetsConfig: updated,
          },
        };
        try {
          localStorage.setItem(STORAGE_KEYS.ORG_DATABASES, JSON.stringify(updatedDbs));
        } catch {}
        return updatedDbs;
      });

      // 2. Update organization list item
      if (cfg.webAppUrl !== undefined || cfg.spreadsheetId !== undefined || cfg.spreadsheetUrl !== undefined) {
        setOrganizations((prevOrgs) => {
          const updatedOrgs = prevOrgs.map((o) =>
            o.id === activeOrganizationId
              ? {
                  ...o,
                  ...(cfg.webAppUrl !== undefined ? { webAppUrl: cfg.webAppUrl } : {}),
                  ...(cfg.spreadsheetId !== undefined ? { spreadsheetId: cfg.spreadsheetId } : {}),
                  ...(cfg.spreadsheetUrl !== undefined ? { spreadsheetUrl: cfg.spreadsheetUrl } : {}),
                }
              : o
          );
          try {
          ...
          saveCloudOrganizations(updatedOrgs).catch((e) => console.warn('Cloud save orgs notice:', e));
          return updatedOrgs;
        });
      }

      // 3. Persist organization-specific sheets config to Cloud
      saveOrgSheetsConfig(activeOrganizationId, updated).catch((e) => console.warn('Cloud save org sheets cfg notice:', e));

      // 4. Save cloud app connection
      saveCloudAppConnection(
        updated,
        organization,
        activeOrganizationId,
        currentUser?.username || undefined
      ).catch((e) => console.warn('Cloud app conn notice:', e));
  ...
  const createGoogleSpreadsheetForApp = async (options?: {
    ...
    // 2. Try REST API if access token is available (or request sensitive OAuth scope if interactive)
    let token = await getAccessToken();
    if (!token && options?.promptForOAuth) {
      try {
        ...
        console.warn('OAuth prompt cancelled or notice:', promptErr);
      }
    }
    ...
    if (token) {
      try {
        ...
        // If it was a scope error and promptForOAuth is requested, prompt user for OAuth approval
        if (isScope && options?.promptForOAuth) {
          try {
            const authRes = await googleSignIn(true);
            if (authRes?.accessToken) {
              setIsGoogleAccountConnected(true);
              setGoogleConnectedEmail(authRes.user.email || undefined);
              const retrySheet = await createAppSpreadsheet(
                authRes.accessToken,
                officeName,
                activeFiscalYear
              );
              const updatedConfig: GoogleSheetsConfig = {
                ...
                spreadsheetName: retrySheet.title,
                autoSync: true,
                syncMode: 'auto',
              };
              setGoogleSheetsConfig(updatedConfig);
              ...
              if (!options?.silent) {
                addToast('success', 'नयाँ सिट सिर्जना भयो', `गुगल ड्राइभमा '${retrySheet.title}' तयार भयो र लिंक गरियो।`);
              }
              await syncWithGoogleSheets('push', { spreadsheetIdOverride: retrySheet.id });
              return {
                success: true,
                spreadsheetId: retrySheet.id,
                url: retrySheet.url,
            ...
            console.warn('OAuth retry cancelled:', retryErr);
          }
        }
  ...
  const syncWithGoogleSheets = async (
    ...
    const activeOrgObj = organizations.find((o) => o.id === activeOrganizationId);
    const activeOrgDb = orgDatabases[activeOrganizationId];
    const resolvedSheetsConfig = activeOrgDb?.googleSheetsConfig || googleSheetsConfig;
    ...
    if (hasWebApp) {
      try {
        ...
        const webAppPayload = {
          ...
          officeName: sanitizedOrg.officeName || sanitizedOrg.name || 'कार्यालय',
          organizationId: activeOrganizationId,
          ...payload,
        };
  ...
  const triggerAutoSyncOnSave = async (overrides?: {
    ...
    const activeOrgObj = organizations.find((o) => o.id === activeOrganizationId);
    const activeOrgDb = orgDatabases[activeOrganizationId];
    const currentConfig = activeOrgDb?.googleSheetsConfig || googleSheetsConfig;

    // Only proceed if autoSync is not explicitly disabled
    if (currentConfig.autoSync === false && googleSheetsConfig.autoSync === false) return;
    ...
    try {
      await syncWithGoogleSheets('push', {
        isAutoSync: true,
        spreadsheetIdOverride: targetSpreadsheetId,
        ...overrides,
      });
    } catch (err) {
  ...
  const handlePostLoginSheetsAutomation = useCallback(
    async (loggedUser: User) => {
      try {
        ...
        // If spreadsheetId is not yet created, attempt background creation quietly
