...
function cleanUserData(data: any): UserEntity {
  ...
  const userObj: UserEntity = {
    ...
    password: data.password || data.metadata?.password || undefined,
    securityPin: data.securityPin || data.metadata?.securityPin || '1234',
    securityQuestion: data.securityQuestion || data.metadata?.securityQuestion || 'तपाईंको पहिलो विद्यालयको नाम के हो?',
    securityAnswer: data.securityAnswer || data.metadata?.securityAnswer || 'नेपाल',
    mustChangePassword: data.mustChangePassword !== undefined ? Boolean(data.mustChangePassword) : (data.metadata?.mustChangePassword !== undefined ? Boolean(data.metadata.mustChangePassword) : false),
    isFirstLogin: data.isFirstLogin !== undefined ? Boolean(data.isFirstLogin) : (data.metadata?.isFirstLogin !== undefined ? Boolean(data.metadata.isFirstLogin) : false),
